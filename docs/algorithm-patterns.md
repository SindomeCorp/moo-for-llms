...
| String scanning and replacement | `examples/utility-packages/string-utils-index-all-adapted.moo`, `examples/strings/replace-substring-loop.moo`, `examples/strings/split-key-value.moo` | `$string_utils` | Portable unless ANSI, xterm, PCRE, or patch-specific helpers are used. |
| Text wrapping and column formatting | `examples/utility-packages/string-utils-side-by-side-adapted.moo`, `examples/strings/ansi-aware-wrap-adapted.moo` | `$string_utils:wrap`, `$string_utils:columnize` style helpers | Plain string formatting can be portable; ANSI-aware display helpers are often core-specific. |
| Parser-style decomposition | `examples/utility-packages/string-utils-parse-argspec-adapted.moo`, `examples/utility-packages/string-utils-parse-verbref-adapted.moo`, `examples/command-parser/` | command and string utility parsers | Portable when implemented in MOO; command-parser behavior still depends on argspec setup. |
